...
public class SyntacticAnalyzer {
    ...
    private void claseConcreta() throws LexicalException, IOException, SyntacticException, SemanticException, SemanticExceptionSimple {
        if (currentToken.getTokenId().equals("pr_class")){
            ...
            if(ancestorToken!= null && SymbolTable.getInstance().getInterface(ancestorToken.getLexeme())!= null){
                currentClass.addAncestorInterface(SymbolTable.getInstance().getInterface(ancestorToken.getLexeme()));
            }
            if (isImplements){
    ...
    private void interface_() throws LexicalException, IOException, SyntacticException, SemanticException {
        if (currentToken.getTokenId().equals("pr_interface")){
            ...
            if (extendsToken!= null && SymbolTable.getInstance().getInterface(extendsToken.getLexeme())!= null){
                currentInterface.addAncestorInterface(SymbolTable.getInstance().getInterface(extendsToken.getLexeme()));
            }
            match("{");
            listaEncabezados();
            match("}");
